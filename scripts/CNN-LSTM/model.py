...
from keras.layers import LSTM, Dropout, MaxPool1D, Conv1D
...
def CNNLSTM(dataset, file_no, Train=False, trj_wise=False, plot=False):
    ...
    with tf.Session() as session:
        ...
        if Train == True:
            ...
            print("No epoches: ", epochs, "No itr: ", no_of_batches)
            __start = time.time()
            for ep in range(epochs):

                for itr in range(no_of_batches):
                    ## training ##
            ...
            if trj_wise:
                ...
                no_of_batches = int(x_validation.shape[0] / (batch_size * sequence_length))
                print("#of validation points:", x_validation.shape[0], "#datapoints covers from minibatch:",
                      batch_size * sequence_length, "iterations/epoch", no_of_batches)
